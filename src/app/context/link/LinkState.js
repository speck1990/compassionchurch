...
const LinkState = props => {
	...
	const getLinks = async () => {
		...
		try {
			const res = await axios.get(`http://${process.env.REACT_APP_URL}/api/links`);
	...
	const addLink = async link => {
		...
		try {
			...
			const res = await axios.post(`http://${process.env.REACT_APP_URL}/api/links`, link, config);
	...
	const updateLink = async link => {
		...
		try {
			...
			const res = await axios.put(`http://${process.env.REACT_APP_URL}/api/links/${link._id}`, link, config);
	...
	const deleteLink = async id => {
		...
		try {
			await axios.delete(`http://${process.env.REACT_APP_URL}/api/links/${id}`);
		} catch (err) {
			dispatch({
				type: LINK_ERROR,
				payload: err
	...
	const setCurrent = async id => {
		...
		if (id) {
			try {
				const res = await axios.get(`http://${process.env.REACT_APP_URL}/api/links/${id}`);
